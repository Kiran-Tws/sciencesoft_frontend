...
export default function InquiryDetail() {
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}/api/inquiries/session/${sessionId}`)
      .then((res) => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white rounded-md shadow-md">
      ...
      <h1 className="text-3xl font-extrabold mb-8 border-b pb-4">
        Inquiry Details
      </h1>
      ...
      <section className="mb-8">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-gray-700">
          <div>
            ...
            <dd>{inquiry.user_details?.name || "N/A"}</dd>
          </div>
          <div>
            <dt className="font-medium">Company:</dt>
            <dd>{inquiry.user_details?.company_name || "N/A"}</dd>
          </div>
          <div>
            <dt className="font-medium">Email:</dt>
            <dd>{inquiry.user_details?.work_email || "N/A"}</dd>
          </div>
          <div>
            <dt className="font-medium">Phone:</dt>
            <dd>{inquiry.user_details?.phone_number || "N/A"}</dd>
          </div>
          <div>
            <dt className="font-medium">Preferred Communication:</dt>
            <dd>{inquiry.user_details?.preferred_communication || "None"}</dd>
          </div>
        </dl>
      </section>
      ...
      <section className="mb-8">
        ...
        <p>
          <strong>Service:</strong> {inquiry.service.name}
        </p>
        <p>
          <strong>Category:</strong> {inquiry.category.name}
        </p>
        <p>
          <strong>Subcategory:</strong> {inquiry.subcategory.name}
        </p>
      </section>
      ...
      <section>
        ...
        <ol className="list-decimal list-inside space-y-4">
          {inquiry.questions.map((q) => (
            <li key={q.id} className="text-gray-900 font-medium">
              {q.question}
